...
{
	public:
		bool CheckCollision(FlappyObject flappy, PipePair pipes)
		{
			UIObject lowerPipe = pipes.GetLower();
			UIObject upperPipe = pipes.GetUpper();
			
			bool collisionLower = flappy.GetStartX() < lowerPipe.GetStartX() + lowerPipe.GetWidth() && 
			...
			bool collisionUpper = flappy.GetStartX() < upperPipe.GetStartX() + upperPipe.GetWidth() &&
